---
title: ""
page-layout: full
toc: false
---

::: {.grid .hero-section}

::: {.g-col-12 .g-col-md-4 .g-col-lg-3 .text-center .hero-sidebar}
# 江斌

<!-- <img src="assets/avatar.png" class="profile-image" /> -->
<img src="assets/avatar.jpg" class="profile-image" />

<div class="hero-links">
  <a href="https://jiangbin-resume.vercel.app/" class="btn btn-primary btn-sm" role="button">
    <i class="bi bi-file-earmark-person"></i> 求职简历
  </a>
  <a href="https://jiangbin-ai.pages.dev" class="btn btn-outline-primary btn-sm" role="button">
    <i class="bi bi-robot"></i> AI 门户
  </a>
  <a href="https://github.com/jiangbingo" class="btn btn-outline-dark btn-sm" role="button">
    <i class="bi bi-github"></i> Github
  </a>
  <a href="https://x.com/jiangbinAI" class="btn btn-outline-dark btn-sm" role="button">
    <i class="bi bi-twitter-x"></i> X
  </a>
  <a href="mailto:jiangbingo@hotmail.com" class="btn btn-outline-secondary btn-sm" role="button">
    <i class="bi bi-envelope"></i> Email
  </a>
</div>

:::

::: {.g-col-12 .g-col-md-8 .g-col-lg-9}

## 个人简介

**AI 应用开发工程师 · Agent EDA Platform @ YFHC**

来自杭州，从嵌入式到 LLM 平台，跨越硬件与 AI 的全栈工程师。当前主导 Agent EDA Platform(LLM 训推一体化平台)开发,聚焦 vLLM/MLX 推理后端、Verilog AI 辅助编程、EDA Agent X86 闭环与模型评测体系。

:::

:::

## 核心能力

::: {.grid .my-4}

::: {.g-col-12 .g-col-md-6 .g-col-lg-4}
<div class="skill-card">
<div class="skill-icon">🤖</div>
<div class="skill-title">AI 应用工程</div>
<div class="skill-desc">RAG 检索增强、Prompt Engineering、Agent RAG、模型评测、vLLM/MLX 推理</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-lg-4}
<div class="skill-card">
<div class="skill-icon">⚙️</div>
<div class="skill-title">Python 后端</div>
<div class="skill-desc">FastAPI、Django、Pydantic、SQLModel、Celery、Redis、异步架构</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-lg-4}
<div class="skill-card">
<div class="skill-icon">☁️</div>
<div class="skill-title">基础设施 & DevOps</div>
<div class="skill-desc">Docker Compose、GitHub Actions、Gitea Actions、Azure ML、Nginx</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-lg-4}
<div class="skill-card">
<div class="skill-icon">🧠</div>
<div class="skill-title">深度学习</div>
<div class="skill-desc">PyTorch 2.2.0+、TensorFlow 2.19.1、YOLOv8、ONNX Runtime</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-lg-4}
<div class="skill-card">
<div class="skill-icon">🔄</div>
<div class="skill-title">自动化 & DevOps</div>
<div class="skill-desc">自动化测试、效率工具、CI/CD 流程、持续集成</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-lg-4}
<div class="skill-card">
<div class="skill-icon">💻</div>
<div class="skill-title">全栈工程背景</div>
<div class="skill-desc">嵌入式开发（C/汇编）、React/TypeScript、Python 全栈、跨领域技术融合</div>
</div>
:::

:::

<div class="my-5"></div>

## 技能栈

::: {.grid}

::: {.g-col-12 .g-col-md-6 .g-col-xl-4}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">AI / 大模型 & RAG</h3>
<ul class="skills-list">
<li><strong>RAG 检索增强</strong>：文档解析、向量检索（Milvus）、Agent RAG</li>
<li><strong>Prompt Engineering</strong>：多轮对话、工具调用、Text-to-SQL</li>
<li><strong>模型推理部署</strong>：vLLM (GPU) / MLX (Apple Silicon)、模型评测 (lm-eval / MT-Bench)</li>
<li><strong>LLM Function Calling</strong>：与工作流编排、Vanna.ai SQL生成</li>
<li><strong>深度学习框架</strong>：PyTorch 2.2.0+、TensorFlow 2.19.1、ONNX Runtime</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-xl-4}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">后端开发 & 数据</h3>
<ul class="skills-list">
<li><strong>Languages</strong>: Python (FastAPI, Django, Flask), <strong>C++</strong></li>
<li><strong>Async</strong>: Celery, Redis, WebSocket, 异步任务调度</li>
<li><strong>DB</strong>: MariaDB, MySQL, Milvus, <strong>PostgreSQL</strong>, SQLModel / Alembic</li>
<li><strong>Data</strong>: Scrapy, Pandas, NumPy, SciPy, UMAP降维, HDBSCAN聚类</li>
<li><strong>API Design</strong>: RESTful API, Pydantic 2.9+, 异步HTTP</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-md-6 .g-col-xl-4}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">基础设施 & 工程化</h3>
<ul class="skills-list">
<li><strong>Azure</strong>: Function, ML Service, Serverless, Web App, Blob Storage</li>
<li><strong>DevOps</strong>: Docker Compose, GitHub Actions, Gitea Actions, Nginx, JIRA</li>
<li><strong>ML DevOps</strong>: wheel包自动构建、CI/CD、模型分发</li>
<li><strong>Test</strong>: RobotFramework, <strong>pytest</strong>, <strong>unittest</strong></li>
<li><strong>Frontend</strong>: React / TypeScript</li>
</ul>
</div>
</div>
:::

:::

<div class="my-5"></div>

## 代表项目

::: {.grid}

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">Agent EDA — LLM 训推一体化平台</h3>
<div class="project-meta">2026.03 – 至今 · 技术负责人 · FastAPI / React 19 / vLLM / MLX</div>
<ul>
<li><strong>全栈 Monorepo</strong>：OpenAI 兼容 Chat API（流式 + Thinking 模式，支持 qwen3/deepseek-r1），模型全生命周期（注册 → 下载 → 部署 → 推理）。</li>
<li><strong>可插拔推理后端</strong>：vLLM (Linux GPU) / MLX (Apple Silicon) 硬件自动切换，GPU 资源动态分配。</li>
<li><strong>Verilog AI 辅助编程</strong>：Monaco Editor + EDA 工具链（iverilog/yosys/verilator）+ AI Agent + WaveDrom 波形查看。</li>
<li><strong>EDA Agent X86 闭环</strong>：AI 服务器调度 + X86 执行节点，SSH 非交互式提交 Innovus/Calibre/VCS 商业 EDA 工具。</li>
<li><strong>模型评测</strong>：lm-eval + MT-Bench + Verilog 专项（HDLBits/VerilogEval/APB-CRC），Celery 四队列异步调度。</li>
<li><strong>工程化</strong>：JWT + LDAP 认证 + RBAC 三级权限 + Docker BuildKit 多阶段 + Gitea Actions CI/CD。</li>
<li><strong>研究产出</strong>：芯片设计 Agent 横评报告（10+ 开源/商业 agent），vLLM 多模型 GPU 配置 benchmark v4.3。</li>
<li><strong>技术栈</strong>：FastAPI、React 19、PostgreSQL、Redis、Celery、vLLM、MLX、Docker Compose、Gitea Actions</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">缺陷检测大模型应用平台</h3>
<div class="project-meta">2025.04 – 至今 · 工业缺陷检测 · FastAPI / Azure / ML</div>
<ul>
<li><strong>后端架构</strong>：搭建缺陷检测系统后端，融合专用视觉模型（YOLOv8）与通用大模型，支持在线缺陷识别与判定。</li>
<li><strong>接口设计</strong>：采用 Pydantic 2.9+ 模型结合 FastAPI 框架，路由 RESTful 化，确保系统高效稳定。</li>
<li><strong>工程化部署</strong>：实现 Function Calling 与 ML 模型工程化部署（本地/云上），优化资源配置，支持 Azure ML Endpoint。</li>
<li><strong>高性能通信</strong>：设计 RabbitMQ 异步消息队列与 WebSocket 通信，提升系统吞吐与用户体验。</li>
<li><strong>技术栈</strong>：PyTorch 2.2.0+、ONNX Runtime 1.16.0+、Celery、RabbitMQ、Azure Web App、Blob Storage</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">RAG 智能问答系统</h3>
<div class="project-meta">2024.11 – 2025.04 · 企业知识库 · RAG / Milvus</div>
<ul>
<li><strong>全栈负责</strong>：负责后端架构、编码、工作流及部署全环节，项目已预上线。</li>
<li><strong>核心技术</strong>：主导 AI 知识库搭建，开发 Prompt 提示词工程，推进 RAG 检索增强生成开发。</li>
<li><strong>数据优化</strong>：使用 Milvus 设计向量数据库结构，显著提升数据处理效率和召回准确性。</li>
<li><strong>Text-to-SQL</strong>：实现自然语言到SQL转换，集成Vanna.ai进行智能查询。</li>
<li><strong>技术栈</strong>：FastAPI、Milvus、PostgreSQL、SQLAlchemy 2.0+、Agent RAG</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">质量分析平台（piyi-api）</h3>
<div class="project-meta">企业项目 · FastAPI + 算法一体化</div>
<ul>
<li><strong>算法集成</strong>：基于 TensorFlow 2.19.1 + Keras 3.11.2 开发质量分析模型，实现 UMAP 降维 + HDBSCAN 聚类。</li>
<li><strong>特征工程</strong>：使用 NumPy、SciPy、Pandas 进行特征提取和数据处理，支持高维数据降维可视化。</li>
<li><strong>混合部署</strong>：支持本地模型和 Azure ML Endpoint 自动切换，实现弹性扩展。</li>
<li><strong>技术栈</strong>：TensorFlow 2.19.1、Keras 3.11.2、UMAP-learn 0.5.9、HDBSCAN 0.8.40、scikit-learn 1.7.2</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">AI推理引擎（AI-Project）</h3>
<div class="project-meta">企业项目 · YOLOv8 ONNX推理</div>
<ul>
<li><strong>模块化设计</strong>：开发可独立安装的Python包（ai_libs + fabric_defect），支持pip一键部署。</li>
<li><strong>高性能推理</strong>：基于 PyTorch 2.2.0+ 和 ONNX Runtime 1.16.0+ 实现 YOLOv8 目标检测，支持GPU加速。</li>
<li><strong>CI/CD自动化</strong>：配置 GitHub Actions 自动构建wheel包和GitHub Release，实现生产级部署。</li>
<li><strong>技术栈</strong>：PyTorch 2.2.0+、ONNX 1.15.0+、OpenCV 4.8.1+、pyproject.toml、GitHub Actions</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">CASPI业务系统（caspi-api）</h3>
<div class="project-meta">企业项目 · 业务API + CI/CD</div>
<ul>
<li><strong>API开发</strong>：基于 FastAPI 开发核心业务API，实现文件上传、批量删除等功能。</li>
<li><strong>CI/CD优化</strong>：重构 GitHub Actions 工作流，提升自动化部署效率。</li>
<li><strong>持续迭代</strong>：22个PR，活跃度高，代码质量优秀（83%合并率）。</li>
<li><strong>技术栈</strong>：FastAPI、Pydantic、GitHub Actions、Azure</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">Genesis大型项目</h3>
<div class="project-meta">企业项目 · Agent RAG实践</div>
<ul>
<li><strong>Agent RAG实践</strong>：AI代理与检索增强生成结合，处理复杂业务逻辑。</li>
<li><strong>大规模系统</strong>：项目体积174MB，多模块、多业务线综合系统开发。</li>
<li><strong>持续迭代</strong>：25次高质量代码提交，系统架构设计经验丰富。</li>
<li><strong>技术栈</strong>：Python、Agent RAG、复杂业务系统</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">3GPP 协议解析器</h3>
<div class="project-meta">NOKIA · 文档解析 · Python</div>
<ul>
<li><strong>技术挑战</strong>：针对 3GPP 协议超大 Word 文档的复杂表格（多维表格）和非结构化数据进行识别、纠错、解析。</li>
<li><strong>核心功能</strong>：开发 Python 文件解析器，自动化识别文本、表格、数据、格式错误并部分纠错，输出 log 文件。</li>
<li><strong>成果</strong>：正确解析并输出结构化数据，大幅提升协议文档处理效率。</li>
<li><strong>技术栈</strong>：Python、文档解析、数据清洗、结构化输出</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">自动化测试与效率工具</h3>
<div class="project-meta">NOKIA · DevOps · Python / Shell</div>
<ul>
<li><strong>云环境自动化工具</strong>：开发自动化预定云平台测试环境的 Python 工具，大幅提升团队工作效率。</li>
<li><strong>效率工具开发</strong>：开发多种自动化工具，包括 Sleeping Cell 检测工具、网络爬虫、Selenium 自动化测试工具等，提升测试与开发效率。</li>
<li><strong>技术栈</strong>：Python、Shell、SQL、Selenium、DevOps</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">电商云平台</h3>
<div class="project-meta">杭州格菱 · Django · 后端开发</div>
<ul>
<li><strong>后台模块开发</strong>：构建数据中心后台模块，基于 Python Django 编写代码，实现后台管理页面。</li>
<li><strong>数据采集</strong>：编写网络爬虫获取和整理 20G 级别的网络资源，构建数据云平台。</li>
<li><strong>成果</strong>：完成电商管理平台 Demo，实现数据采集、处理和展示全流程。</li>
<li><strong>技术栈</strong>：Python、Django、爬虫、数据处理</li>
</ul>
</div>
</div>
:::

::: {.g-col-12 .g-col-lg-6}
<div class="card h-100">
<div class="card-body">
<h3 class="card-title">新标准扶梯项目</h3>
<div class="project-meta">西子优迈 · 嵌入式开发 · C / 汇编</div>
<ul>
<li><strong>嵌入式开发</strong>：基于 PLC、MCU、ARM、DSP 的逻辑软件编程（C、ASM 汇编）。</li>
<li><strong>测试工装</strong>：开发板和嵌入式系统的测试工装制作与维护，以及安全电子系统认证。</li>
<li><strong>技术指导</strong>：提供先锋产品技术指导，收集并解决工地现场问题。</li>
<li><strong>主要业绩</strong>：完成安全电子系统 SIL 2 认证；完成国内新国标自动扶梯首批产品的发布。</li>
<li><strong>技术栈</strong>：C、汇编、PLC、MCU、ARM、DSP</li>
</ul>
</div>
</div>
:::

:::

<div class="my-5"></div>

## 工作经历

<div class="timeline">

<div class="timeline-item">
<div class="timeline-title">亿方杭创 · AI 应用开发工程师（Agent EDA Platform 技术负责人）</div>
<div class="timeline-meta">2026.03 – 至今 · 杭州</div>
<ul>
<li><strong>平台主导</strong>：担任 Agent EDA Platform（LLM 训推一体化平台）技术负责人，主导架构设计、技术选型与团队分工。</li>
<li><strong>推理引擎</strong>：实现 OpenAI 兼容 Chat API（流式 + Thinking 模式），设计可插拔推理后端（vLLM GPU / MLX Apple Silicon 硬件级自动切换）。</li>
<li><strong>模型全生命周期</strong>：搭建注册 → ModelScope 下载 → 部署 → 推理全流程，GPU 资源动态分配，模型评测体系（lm-eval + MT-Bench + Verilog 专项）。</li>
<li><strong>Verilog AI 辅助编程</strong>：开发 Verilog 在线 IDE（Monaco Editor + EDA 工具链 + AI Agent + WaveDrom 波形查看）。</li>
<li><strong>EDA Agent X86 闭环</strong>：设计 AI 服务器调度 + X86 执行节点的分布式架构，SSH 非交互式提交商业 EDA 工具（Innovus/Calibre/VCS）。</li>
<li><strong>工程化</strong>：Celery 四队列异步调度 + JWT/LDAP 认证 + RBAC 三级权限 + Docker BuildKit 多阶段构建 + Gitea Actions CI/CD。</li>
<li><strong>研究产出</strong>：芯片设计 Agent 横评报告（10+ 开源/商业 agent），vLLM 多模型 GPU 配置 benchmark，算力评估报告。</li>
<li><strong>技术栈</strong>：FastAPI、React 19、PostgreSQL、Redis、Celery、vLLM、MLX、Docker Compose、Gitea Actions</li>
</ul>
</div>

<div class="timeline-item">
<div class="timeline-title">杭州克雷登工业 · AI 应用工程师</div>
<div class="timeline-meta">2024.11 – 2026.03</div>
<ul>
<li><strong>后端架构</strong>：负责后端设计与开发，采用 Pydantic + FastAPI，确保系统高效稳定。</li>
<li><strong>AI系统开发</strong>：设计开发智能问答系统，专注于 Prompt 工程及 RAG 技术选型实践，提升问答准确率与语义理解效果。</li>
<li><strong>ML工程化</strong>：开发缺陷检测机器学习算法，结合大模型应用进行工程化实践。</li>
<li><strong>云平台部署</strong>：负责 Azure 云平台系统（后端、Function Calling、ML）的工程化部署与性能优化。</li>
<li><strong>主要业绩</strong>：主导 FastAPI 项目重构，引入 Pydantic 实现数据模型校验，提升接口稳定性与可维护性；完成 RESTful 风格路由设计，提高前后端协作效率；负责数据库、向量数据库、缓存数据库等选型与架构设计，优化数据存储结构，支持高并发读写场景</li>
</ul>
</div>

<div class="timeline-item">
<div class="timeline-title">杭州人本集团 · Python 后端开发</div>
<div class="timeline-meta">2024.08 – 2024.10 · 数字化 MES 平台</div>
<ul>
<li><strong>技术规划</strong>：负责智能化数字化 MES 平台后台模块的技术规划。</li>
<li><strong>后端开发</strong>：负责业务逻辑编码实现（Django），设计异步 Celery 任务与 MariaDB 数据库结构。</li>
<li><strong>API设计</strong>：参与 API 接口设计与评审，编写技术文档与接口规范。</li>
<li><strong>主要业绩</strong>：编写 API 接口文档与接口规范；完成后端业务逻辑编码；进行 MES 系统设计</li>
</ul>
</div>

<div class="timeline-item">
<div class="timeline-title">诺基亚通信技术有限公司 · 测试开发工程师</div>
<div class="timeline-meta">2016.10 – 2023.05</div>
<ul>
<li><strong>自动化测试</strong>：基于 RobotFramework (灰盒) 和 TTCN-3 语言的测试项目。</li>
<li><strong>软件开发</strong>：参与 C++ 软件开发与测试。</li>
<li><strong>工具开发</strong>：基于 Python、Shell、Lua 开发自动化测试与平台工具，开发云环境自动化预定工具提升工作效率。</li>
<li><strong>DevOps 实践</strong>：使用 Jenkins、GitLab CI、Docker 等工具优化交付流程。</li>
</ul>
</div>

<div class="timeline-item">
<div class="timeline-title">杭州格菱科技有限公司 · Python 开发</div>
<div class="timeline-meta">2014.12 – 2016.10 · 电商云平台</div>
<ul>
<li>基于 Python Django 搭建数据云平台。</li>
<li>负责爬虫数据采集与处理。</li>
</ul>
</div>

<div class="timeline-item">
<div class="timeline-title">西子优迈 · 软件开发工程师</div>
<div class="timeline-meta">2010.07 – 2014.09 · 新标准扶梯项目</div>
<ul>
<li>负责基于 PLC、MCU、ARM、DSP 的逻辑软件编程（C、ASM 汇编）。</li>
<li>负责开发板和嵌入式系统的测试工装制作与维护，以及安全电子系统认证。</li>
<li>提供先锋产品技术指导，收集并解决工地现场问题。</li>
<li><strong>主要业绩</strong>：完成安全电子系统 SIL 2 认证；完成国内新国标自动扶梯首批产品的发布。</li>
</ul>
</div>

</div>

<div class="my-5"></div>

## 教育与证书

- **中南民族大学** · 本科 · 电子信息工程类 – 生物医学工程 · 2005 – 2010
- 大学英语六级

<div class="my-5"></div>


## 志愿与社区

<div class="timeline">

<div class="timeline-item">
<div class="timeline-title">西湖创客汇 · 创客导师</div>
<div class="timeline-meta">2015.06 – 2017.05</div>
服务 1000+ 青年创客社区，分享技术经验，指导项目开发；组织并主持多场技术工作坊和黑客马拉松活动。
</div>

<div class="timeline-item">
<div class="timeline-title">1566 创客公益基金 · 爱心大使</div>
<div class="timeline-meta">核心成员</div>
作为核心成员，为有潜力的创客项目提供公益基金与技术支持；参与项目评审，帮助筛选和培养有社会价值的创新项目。
</div>

</div>

<div class="my-5"></div>

## 联系我

::: {.grid}

::: {.g-col-12 .g-col-md-8}
欢迎就 Python / AI 应用工程化 / RAG 系统 / 工业智能化等方向进行交流或合作。

- **邮箱**：[jiangbingo@hotmail.com](mailto:jiangbingo@hotmail.com)
- **GitHub**：[github.com/jiangbingo](https://github.com/jiangbingo)
- **AI 门户**：[jiangbin-ai.pages.dev](https://jiangbin-ai.pages.dev)
- **X（Twitter）**：[x.com/jiangbinAI](https://x.com/jiangbinAI)
- **所在城市**：杭州 / 武汉
:::

::: {.g-col-12 .g-col-md-4}
<img src="assets/wechat.jpg" style="width: 100%; max-width: 180px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" alt="WeChat QR Code">
:::

:::
:::